...
export async function updateInvoice(id, changes) {
  ...
  const next = normalizeInvoice({
    ...
    // Repairs logged from a new invoice are linked after they're created.
    ticketIds: Array.isArray(changes?.ticketIds) ? changes.ticketIds : current.ticketIds,
  });
  const updated = await sql`
    UPDATE custom_invoices
    SET ticket_id = ${next.ticketIds[0] || ""},
        customer_name = ${next.billTo.name},
...
/** Adds a sent email to the invoice's log (newest last, 50 kept). */
export async function recordInvoiceEmail(id, entry) {
  const rows = await sql`
    UPDATE custom_invoices
    SET emailed_at = now(),
        email_log = (COALESCE(email_log, '[]'::jsonb) || ${JSON.stringify([entry])}::jsonb)
    WHERE id = ${String(id || "").trim()} AND deleted_at IS NULL
    RETURNING *
  `;
  if (!rows.length) throw new Error("Invoice not found");
  return rowToInvoice(rows[0]);
...
function rowToInvoice(row) {
  ...
  return {
    ...
    emails: Array.isArray(row.email_log) ? row.email_log : [],
