...
export async function updateInvoice(id, changes) {
  ...
  const rows = await sql`SELECT * FROM custom_invoices WHERE id = ${key}`;
...
export async function getInvoiceById(id) {
  const rows = await sql`SELECT * FROM custom_invoices WHERE id = ${text(id)}`;
  if (!rows.length) throw new Error("Invoice not found");
  return rowToInvoice(rows[0]);
}
...
export async function listInvoices() {
  const rows = await sql`SELECT * FROM custom_invoices ORDER BY created_at DESC LIMIT 2000`;
...
export async function getInvoiceForTicket(ticketId) {
  ...
  const rows = await sql`
    ...
    WHERE ticket_id = ${key} OR payload->'ticketIds' @> ${JSON.stringify([key])}::jsonb
