...
{"vendor": string, "taxAmount": number, "parts": [{"part": string, "quantity": integer, "unitCost": number}]}
...
- "taxAmount" is the total sales tax charged on the whole order (e.g. a line like "Sales Tax 7%: $15.57" means taxAmount is 15.57), as a plain number. Do NOT include shipping charges in this figure. If the document shows no sales tax at all, use 0.
- If you can't find any line items at all, return {"vendor": "", "taxAmount": 0, "parts": []}.`;
  ...
  return JSON.stringify({
    ...
    generationConfig: {
      ...
      responseJsonSchema: {
        ...
        required: ["vendor", "taxAmount", "parts"],
      },
    },
  });
}
...
// The invoice's per-unit prices are pre-tax, but nothing downstream of this
// (parts_orders, the collect-back expense, the shipment total shown in the
// review modal) has a separate tax field — it's all just quantity × unitCost.
// So rather than silently under-recording what the shop actually paid, the
// tax gets folded into each line's unitCost, split in proportion to that
// line's share of the pre-tax subtotal (a $130 line carries more of the tax
// than an $8 one, same as the invoice itself would apply it). Left as 0 when
// there's nothing to distribute — a free item, or a subtotal of 0 — rather
// than divide by zero.
function distributeTax(parts, taxAmount) {
  if (!(taxAmount > 0)) return parts;
  const subtotalCents = parts.reduce((sum, p) => sum + Math.round(p.unitCost * 100) * p.quantity, 0);
  if (subtotalCents <= 0) return parts;
  const taxCents = Math.round(taxAmount * 100);
  return parts.map((p) => {
    const lineCents = Math.round(p.unitCost * 100) * p.quantity;
    const lineTaxCents = Math.round(taxCents * (lineCents / subtotalCents));
    const newUnitCents = Math.round((lineCents + lineTaxCents) / p.quantity);
    return { ...p, unitCost: newUnitCents / 100 };
  });
}
...
export function parseExtractionResult(raw) {
  ...
  const rawTaxAmount = Number(parsed.taxAmount);
  const taxAmount = isFinite(rawTaxAmount) && rawTaxAmount > 0 ? Math.round(rawTaxAmount * 100) / 100 : 0;
  return {
    vendor: String(parsed.vendor || "").trim(),
    // Already folded into each line's unitCost below — surfaced here too so
    // the caller can tell the reviewer their unit costs aren't the invoice's
    // pre-tax sticker price without them having to do that math themselves.
    taxAmount,
    parts: distributeTax(parts, taxAmount),
  };
}
