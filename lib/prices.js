...
export async function recordCommonSearchUse({ id, query } = {}) {
  ...
  const key = commonSearchKey(cleanQuery);
  await logSearchEvent(cleanId, cleanQuery);
  if (cleanId) {
    const rows = await sql`
...
// Dated record of each search (Smart restock). Saved-search clicks arrive
// with only an id, so their text is looked up. Best effort — a logging
// failure never breaks the search itself. Old rows are trimmed occasionally.
async function logSearchEvent(id, query) {
  try {
    let text = query;
    if (!text && id) {
      const rows = await sql`SELECT query FROM price_common_searches WHERE id = ${id}`;
      text = rows[0]?.query || "";
    }
    const key = commonSearchKey(text);
    if (!key) return;
    await sql`INSERT INTO price_search_events (query_key, query) VALUES (${key}, ${text.slice(0, 120)})`;
    if (Math.random() < 0.02) {
      await sql`DELETE FROM price_search_events WHERE created_at < now() - interval '400 days'`;
    }
  } catch (err) {
    console.error("Couldn't log price search:", err);
  }
}

/** Searches per phrase over the last `days` days (Smart restock). */
export async function searchDemand(days = 60) {
  const span = Math.min(400, Math.max(1, Math.round(Number(days) || 60)));
  const rows = await sql`
    SELECT query_key, MAX(query) AS query, COUNT(*)::int AS count, MAX(created_at) AS last_at
    FROM price_search_events
    WHERE created_at >= now() - (${span} || ' days')::interval
    GROUP BY query_key
    ORDER BY count DESC
    LIMIT 2000
  `;
  const first = await sql`SELECT MIN(created_at) AS first_at FROM price_search_events`;
  return {
    days: span,
    trackingSince: first[0]?.first_at ? new Date(first[0].first_at).toISOString() : null,
    searches: rows.map((r) => ({ query: r.query, count: r.count, lastAt: r.last_at ? new Date(r.last_at).toISOString() : null })),
  };
}
