...
async function deliver(sender, message) {
  ...
  // Measured before sending: nodemailer converts attachment content in place.
  const attachmentSizes = (message.attachments || []).map((a) => ({ filename: a.filename, bytes: a.content.length }));
  try {
    const info = await transport.sendMail({ from, ...message });
    if (process.env.EMAIL_DRY_RUN === "1") {
      dryRunOutbox.push({ from, ...message, attachments: attachmentSizes });
    }
