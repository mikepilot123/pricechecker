...
export async function getPartsOrder(id) {
...
export async function searchPartsOrders({
  ...
  const vendorLike = likePattern(v);
  const rows = await sql`
    SELECT * FROM parts_orders
    WHERE deleted_at IS NULL
      AND (${q} = '' OR part ILIKE ${like} OR vendor ILIKE ${like} OR shipment_name ILIKE ${like} OR notes ILIKE ${like} OR id ILIKE ${like})
      AND (${v} = '' OR vendor ILIKE ${vendorLike})
      AND (${st} = '' OR status = ${st})
      AND (${unreceivedOnly} = false OR (status <> 'arrived' AND status <> 'cancelled' AND inventory_stock_state <> 'stocked'))
