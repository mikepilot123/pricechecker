...
export async function addBankTransaction(input = {}) {
  ...
  const rows = await sql`
    INSERT INTO bank_transactions (id, occurred_at, kind, account_type, amount, category, reference, notes)
    VALUES (${id}, ${occurredAt}, ${kind}, ${accountType}, ${amount}, ${category}, ${reference}, ${notes})
    RETURNING *
  `;
