...
function collectionBankId(expenseId, collectionId) { return "BANK_COLLECTION_" + expenseId + "_" + collectionId; }
...
// Each collection receipt has a deterministic bank-ledger row. Keeping this
// inside the same transaction as the expense update makes retries idempotent
// and ensures an undo also removes the matching withdrawal.
function collectionBankQueries(id, before, after) {
  const previous = new Map(before.collections.map((entry) => [entry.id, entry]));
  const queries = [];
  for (const entry of after.collections) {
    const oldEntry = previous.get(entry.id);
    if (!entry.voidedAt && !oldEntry) {
      const bankId = collectionBankId(id, entry.id);
      const reference = after.vendor || after.reclaimFrom || after.id;
      const notes = "Cash collected for expense " + after.id
        + (after.reclaimFrom ? " from " + after.reclaimFrom : "");
      queries.push(sql`
        INSERT INTO bank_transactions (id, occurred_at, kind, amount, category, reference, notes)
        SELECT ${bankId}, ${entry.collectedAt}, 'withdrawal', ${entry.amount}, ${after.category}, ${reference}, ${notes}
        FROM expenses
        WHERE id = ${id} AND deleted_at IS NULL AND EXISTS (
          SELECT 1 FROM jsonb_array_elements(reclaim_collections) receipt
          WHERE receipt->>'id' = ${entry.id} AND NULLIF(receipt->>'voidedAt', '') IS NULL
        )
        ON CONFLICT (id) DO NOTHING
      `);
    } else if (entry.voidedAt && oldEntry && !oldEntry.voidedAt) {
      const bankId = collectionBankId(id, entry.id);
      queries.push(sql`
        UPDATE bank_transactions SET deleted_at = COALESCE(deleted_at, now()), updated_at = now()
        WHERE id = ${bankId} AND EXISTS (
          SELECT 1 FROM expenses, jsonb_array_elements(reclaim_collections) receipt
          WHERE expenses.id = ${id} AND receipt->>'id' = ${entry.id}
            AND NULLIF(receipt->>'voidedAt', '') IS NOT NULL
        )
      `);
    }
  }
...
async function changeExpense(id, change) {
  ...
  for (let attempt = 0; attempt < 4; attempt++) {
    ...
    const [updated] = await sql.transaction([
      ...
      `,
      ...reminderQueries(id),
      ...collectionBankQueries(id, before, expense),
    ]);
