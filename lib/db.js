...
export function ensureSchema() {
  if (!schemaPromise) {
    schemaPromise = (async () => {
      ...
      // Optional promised date for a repair. Tickets older than one day in an
      // active work status also get a shared Reminders entry to prompt a check.
      await sql`ALTER TABLE tickets ADD COLUMN IF NOT EXISTS repair_due_date DATE`;
      // How a repair was paid for, and the card takings row it created — see
      // migrations/026_add_ticket_payment_method.sql and lib/card-payments.js.
      await sql`ALTER TABLE tickets ADD COLUMN IF NOT EXISTS payment_method TEXT NOT NULL DEFAULT ''`;
      await sql`ALTER TABLE tickets ADD COLUMN IF NOT EXISTS card_payment_id TEXT`;
      await sql`ALTER TABLE tickets ADD COLUMN IF NOT EXISTS payment_card_type TEXT NOT NULL DEFAULT ''`;
      // Links a device added to an earlier check-in back to that check-in's
      // ticket — see migrations/036_add_ticket_checkin_group.sql.
      await sql`ALTER TABLE tickets ADD COLUMN IF NOT EXISTS checkin_group TEXT NOT NULL DEFAULT ''`;
      await sql`
      ...
      // Internal running note log for tickets — see
      // migrations/022_create_ticket_notes.sql. Deliberately separate from
      // tickets.notes, which is invoice-visible.
      await sql`
      ...
      `;
      await sql`CREATE INDEX IF NOT EXISTS idx_ticket_notes_ticket ON ticket_notes (ticket_id, created_at DESC)`;
      ...
      // Zoho-style sequential invoice numbers (INV-000090, …) — see
      // migrations/037_create_invoice_number_seq.sql.
      await sql`CREATE SEQUENCE IF NOT EXISTS invoice_number_seq START WITH 90`;
      // Deleted invoices are hidden, not dropped — see lib/invoices.js deleteInvoice().
      await sql`ALTER TABLE custom_invoices ADD COLUMN IF NOT EXISTS deleted_at TIMESTAMPTZ`;
      // Invoice email: linked sender mailboxes and a per-invoice sent log —
      // see lib/email.js and migrations/039_create_email_accounts.sql.
      await sql`
      ...
      `;
      await sql`ALTER TABLE custom_invoices ADD COLUMN IF NOT EXISTS email_log JSONB NOT NULL DEFAULT '[]'::jsonb`;
      ...
      // Reminders tab — see migrations/016_create_reminders.sql.
      await sql`
      ...
      `;
      await sql`CREATE INDEX IF NOT EXISTS idx_reminders_open ON reminders (deleted_at, done, due_at)`;
      // See migrations/017_add_reminder_fields.sql.
      await sql`ALTER TABLE reminders ADD COLUMN IF NOT EXISTS assignee TEXT NOT NULL DEFAULT ''`;
      await sql`ALTER TABLE reminders ADD COLUMN IF NOT EXISTS priority TEXT NOT NULL DEFAULT ''`;
      await sql`ALTER TABLE reminders ADD COLUMN IF NOT EXISTS ticket_id TEXT`;
      await sql`ALTER TABLE reminders ADD COLUMN IF NOT EXISTS ticket_label TEXT NOT NULL DEFAULT ''`;
      // See migrations/020_add_expense_cash_reclaim.sql. Cash fronted for an
      // expense (typically out of the owner's other business) that has to be
      // collected back; lib/expenses.js mirrors each one into a reminder.
      await sql`ALTER TABLE expenses ADD COLUMN IF NOT EXISTS cash_reclaim BOOLEAN NOT NULL DEFAULT FALSE`;
      await sql`ALTER TABLE expenses ADD COLUMN IF NOT EXISTS reclaim_from TEXT NOT NULL DEFAULT ''`;
      await sql`ALTER TABLE expenses ADD COLUMN IF NOT EXISTS reclaim_due_at TIMESTAMPTZ`;
      await sql`ALTER TABLE expenses ADD COLUMN IF NOT EXISTS reclaimed_at TIMESTAMPTZ`;
      // NULL preserves the legacy collected/not-collected state until first edit.
      await sql`ALTER TABLE expenses ADD COLUMN IF NOT EXISTS reclaimed_amount NUMERIC(12, 2)`;
      await sql`ALTER TABLE expenses ADD COLUMN IF NOT EXISTS reclaim_collections JSONB NOT NULL DEFAULT '[]'::jsonb`;
      await sql`CREATE INDEX IF NOT EXISTS idx_expenses_cash_reclaim ON expenses (deleted_at, cash_reclaim, reclaimed_at)`;
      await sql`ALTER TABLE reminders ADD COLUMN IF NOT EXISTS kind TEXT NOT NULL DEFAULT ''`;
      await sql`ALTER TABLE reminders ADD COLUMN IF NOT EXISTS amount NUMERIC(12, 2)`;
      await sql`ALTER TABLE reminders ADD COLUMN IF NOT EXISTS expense_id TEXT`;
      // See migrations/021_add_appointment_reminder_link.sql — every
      // appointment gets a mirrored "coming up" reminder; see
      // lib/appointments.js's syncAppointmentReminder().
      await sql`ALTER TABLE reminders ADD COLUMN IF NOT EXISTS appointment_id TEXT`;
      await sql`CREATE INDEX IF NOT EXISTS idx_reminders_appointment ON reminders (appointment_id) WHERE appointment_id IS NOT NULL`;
      ...
      // Parts ordered from suppliers — see migrations/028_create_parts_orders.sql
      // and lib/parts-orders.js. One row per part; batch_id groups rows
      // uploaded from the same PDF (see lib/parts-order-extraction.js).
      await sql`
      ...
      `;
      await sql`ALTER TABLE parts_orders ADD COLUMN IF NOT EXISTS shipment_name TEXT NOT NULL DEFAULT ''`;
      await sql`ALTER TABLE parts_orders ADD COLUMN IF NOT EXISTS inventory_item_key TEXT`;
      await sql`ALTER TABLE parts_orders ADD COLUMN IF NOT EXISTS inventory_item_label TEXT NOT NULL DEFAULT ''`;
      await sql`ALTER TABLE parts_orders ADD COLUMN IF NOT EXISTS inventory_stocked_quantity INTEGER`;
      await sql`ALTER TABLE parts_orders ADD COLUMN IF NOT EXISTS inventory_stocked_at TIMESTAMPTZ`;
      await sql`ALTER TABLE parts_orders ADD COLUMN IF NOT EXISTS inventory_stock_state TEXT NOT NULL DEFAULT ''`;
      await sql`ALTER TABLE parts_orders ADD COLUMN IF NOT EXISTS payment_status TEXT NOT NULL DEFAULT 'pending'`;
      await sql`CREATE INDEX IF NOT EXISTS idx_parts_orders_batch ON parts_orders (batch_id)`;
      await sql`CREATE INDEX IF NOT EXISTS idx_parts_orders_customer ON parts_orders (customer_id)`;
      await sql`CREATE INDEX IF NOT EXISTS idx_parts_orders_ticket ON parts_orders (ticket_id)`;
      // Card takings ledger — see migrations/023_create_card_payments.sql and
      // 024_create_payouts.sql. The shop's card machine is registered to the
      // owner's brother, so every swipe is money sitting in his account until
      // he transfers it across; these two tables track what is owed and what
      // has been collected. State is derived, never stored — lib/card-payments.js.
      await sql`
        CREATE TABLE IF NOT EXISTS card_payments (
          id           TEXT PRIMARY KEY,
          business     TEXT NOT NULL DEFAULT 'jq',
      ...
      `;
      await sql`CREATE INDEX IF NOT EXISTS idx_card_payments_owed ON card_payments (deleted_at, voided_at, payout_id, settles_at)`;
      await sql`CREATE INDEX IF NOT EXISTS idx_card_payments_taken ON card_payments (deleted_at, taken_at DESC)`;
      await sql`CREATE INDEX IF NOT EXISTS idx_card_payments_ticket ON card_payments (ticket_id)`;
      await sql`
      ...
      `;
      await sql`CREATE INDEX IF NOT EXISTS idx_payouts_paid_at ON payouts (deleted_at, paid_at DESC)`;
      // Actual shop bank balance. Deposits and withdrawals are deliberately a
      // small standalone ledger: card takings remain available in their own
      // detail panel without being mistaken for money already in this account.
      await sql`
      ...
      `;
      await sql`CREATE INDEX IF NOT EXISTS idx_bank_transactions_date ON bank_transactions (deleted_at, occurred_at DESC)`;
      await sql`ALTER TABLE bank_transactions ADD COLUMN IF NOT EXISTS expense_id TEXT`;
      await sql`CREATE INDEX IF NOT EXISTS idx_bank_transactions_expense ON bank_transactions (expense_id) WHERE expense_id IS NOT NULL`;
      // Cash on hand tracked in the same ledger as the bank account, kept
      // apart only by this column — a deposit or withdrawal is either one,
      // never both. Existing rows default to 'bank': every transaction ever
      // recorded before this column existed was a bank one, since cash
      // wasn't tracked at all.
      await sql`ALTER TABLE bank_transactions ADD COLUMN IF NOT EXISTS account_type TEXT NOT NULL DEFAULT 'bank'`;
      // Depositing cash into the bank is one real-world event recorded as two
      // linked rows (a cash withdrawal, a bank deposit) — this column pairs
      // them so editing or deleting one side keeps the other in sync.
      await sql`ALTER TABLE bank_transactions ADD COLUMN IF NOT EXISTS transfer_id TEXT`;
      // Shared settings — see migrations/025_create_app_settings.sql and
      // lib/settings.js. Backs the card-machine fee rates, which staff need to
      // change themselves and every device has to agree on.
      await sql`
      ...
      // Shared saved searches in the Prices tab. Staff can pin the common
      // questions they answer every day, and clicks keep the most-used ones
      ...
      `;
      await sql`CREATE INDEX IF NOT EXISTS idx_price_common_searches_active ON price_common_searches (deleted_at, use_count DESC, last_used_at DESC, updated_at DESC)`;
      // One row per Prices search, with its time — price_common_searches only
      // keeps an all-time count. Powers Smart restock's "searched N× in the
      // last 30 days". See migrations/040_create_price_search_events.sql.
      await sql`
      ...
      `;
      await sql`CREATE INDEX IF NOT EXISTS idx_price_search_events_created ON price_search_events (created_at DESC)`;
