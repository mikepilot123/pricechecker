...
export function ensureSchema() {
  if (!schemaPromise) {
    schemaPromise = (async () => {
      ...
      // month's recognized revenue. lib/tickets.js adjusts this additively
      // (on ticket creation and on repair-cost corrections), never by
      // recomputing from the live tickets table.
      await sql`
      ...
      `;
      // One-time backfill from whatever ticket history already exists
      // (including previously soft-deleted/cleared tickets, since those
      // still represent revenue that happened) so shops upgrading to this
      // ledger see their real history immediately instead of starting empty.
      const [{ count: monthlySalesRows }] = await sql`SELECT COUNT(*)::int AS count FROM monthly_sales`;
      if (monthlySalesRows === 0) {
        await sql`
