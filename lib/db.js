...
export function ensureSchema() {
  if (!schemaPromise) {
    schemaPromise = (async () => {
      ...
      // See migrations/023_add_expense_reclaimed_amount.sql. How much of the
      // fronted cash has actually come back; reclaimed_at stays the "fully
      // settled" marker, set once this reaches the expense amount.
      await sql`ALTER TABLE expenses ADD COLUMN IF NOT EXISTS reclaimed_amount NUMERIC(12, 2) NOT NULL DEFAULT 0`;
      await sql`UPDATE expenses SET reclaimed_amount = amount WHERE reclaimed_at IS NOT NULL AND reclaimed_amount = 0`;
