...
export function ensureSchema() {
  if (!schemaPromise) {
    schemaPromise = (async () => {
      ...
      // existed gets lost. Guarded by NOT EXISTS so it only ever runs once
      // per lead, even though ensureSchema runs on every cold start.
      await sql`
        INSERT INTO lead_notes (lead_id, note, created_at)
        SELECT id, notes, updated_at FROM leads
        WHERE notes <> '' AND NOT EXISTS (
          SELECT 1 FROM lead_notes WHERE lead_notes.lead_id = leads.id
        )
      `;
