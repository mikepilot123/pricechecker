...
export async function addTicket(p) {
  ...
  await sql`
    INSERT INTO tickets (id, customer_name, phone, email, device, issues, status, notes, repair_cost, amount_paid)
    VALUES (${id}, ${snapshot.customerName}, ${snapshot.phone}, ${snapshot.email}, ${snapshot.device}, ${snapshot.issues}, ${snapshot.status}, ${snapshot.notes}, ${snapshot.repairCost}, ${snapshot.amountPaid})
  `;
...
export async function updateTicket(p) {
  ...
  if (changed) {
    await insertVersion(p.id, nextVersion, lines.join("; "), "update", {
      customerName, phone, email, device, issues, status, notes, repairCost, amountPaid,
    });
  }
...
export async function restoreBackup(p) {
  ...
  for (const t of target) {
    ...
    if (existing.length) {
      ...
      await sql`
        INSERT INTO tickets (id, customer_name, phone, email, device, issues, status, notes, repair_cost, amount_paid, created_at)
        VALUES (${t.id}, ${snap.customerName}, ${snap.phone}, ${snap.email}, ${snap.device}, ${snap.issues}, ${snap.status}, ${snap.notes}, ${snap.repairCost}, ${snap.amountPaid}, ${t.created || new Date().toISOString()})
      `;
