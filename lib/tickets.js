...
async function rowToTicket(row) {
  ...
  return {
    ...
    cardType: row.payment_card_type || "",
    cardPaymentId: row.card_payment_id || "",
    checkinGroup: row.checkin_group || "",
  };
}

// ---- Card takings link ------------------------------------------------------
// The card payments/payouts ledger this used to feed has been retired (no
// more UI to view or settle it), so "card" is no longer an accepted payment
// method — nothing new should route into that now-invisible ledger. A ticket
// that already has "card" on file keeps working exactly as before: the
// syncTicketCardPayment() call below still reaches it, just via the
// current-value fallback in the "amount changed, method untouched" branch in
// updateTicket() rather than through this allow-list.
...
/** Read a single active ticket. Does not run reminder side-effects. */
export async function getTicketById(id) {
  const ticketId = String(id || "").trim();
  if (!ticketId) throw new Error("Ticket ID is required");
  const rows = await sql`SELECT * FROM tickets WHERE id = ${ticketId} AND deleted_at IS NULL`;
  if (!rows.length) return null;
  return rowToTicket(rows[0]);
}

/**
 * Filtered ticket lookup for MCP and other callers that must not dump the
 * whole shop. Skips reminder sync so a read cannot write.
 */
export async function searchTickets({ query = "", status = "", limit = 15 } = {}) {
  const q = String(query || "").trim();
  const st = String(status || "").trim();
  const n = Math.min(Math.max(Number(limit) || 15, 1), 40);
  const like = likePattern(q);
  const rows = await sql`
    SELECT * FROM tickets
    WHERE deleted_at IS NULL
      AND (${q} = '' OR customer_name ILIKE ${like} OR device ILIKE ${like} OR issues ILIKE ${like} OR id ILIKE ${like})
      AND (${st} = '' OR status = ${st})
...
export async function addTicket(p) {
  ...
  const checkinGroup = String(p.checkinGroup || "").trim().slice(0, 64);

  try {
    await sql`
      INSERT INTO tickets (id, customer_name, phone, email, device, issues, status, notes, repair_cost, amount_paid, technician, repair_due_date, inventory_item_key, inventory_item_label, inventory_section, inventory_quantity_delta, waiting_for_parts_since, checkin_group)
      VALUES (${id}, ${snapshot.customerName}, ${snapshot.phone}, ${snapshot.email}, ${snapshot.device}, ${snapshot.issues}, ${snapshot.status}, ${snapshot.notes}, ${snapshot.repairCost}, ${snapshot.amountPaid}, ${snapshot.technician}, ${snapshot.repairDueDate}, ${snapshot.inventoryItemKey}, ${snapshot.inventoryItemLabel}, ${snapshot.inventorySection}, ${snapshot.inventoryQuantityDelta}, ${waitingForPartsSince}, ${checkinGroup})
    `;
