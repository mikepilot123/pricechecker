...
// Additive-only: credited when a ticket's repair cost is first set, adjusted
// by the delta when it's later corrected. Never touched by delete/clear/
// restore, so a month's recognized revenue survives housekeeping on the
// live tickets table. See lib/db.js's ensureSchema() for the table + backfill.
...
async function adjustMonthlySales(monthKey, salesDelta, countDelta) {
  if (!salesDelta && !countDelta) return;
  await sql`
...
export async function addTicket(p) {
  ...
  try {
    await sql`
      INSERT INTO tickets (id, customer_name, phone, email, device, issues, status, notes, repair_cost, amount_paid, technician, inventory_item_key, inventory_item_label, inventory_section, inventory_quantity_delta)
      VALUES (${id}, ${snapshot.customerName}, ${snapshot.phone}, ${snapshot.email}, ${snapshot.device}, ${snapshot.issues}, ${snapshot.status}, ${snapshot.notes}, ${snapshot.repairCost}, ${snapshot.amountPaid}, ${snapshot.technician}, ${snapshot.inventoryItemKey}, ${snapshot.inventoryItemLabel}, ${snapshot.inventorySection}, ${snapshot.inventoryQuantityDelta})
    `;
    await insertVersion(id, 1, `Logged — ${status}`, "create", snapshot);
    await recordInventoryMovement(id, inventoryMovement, "ticket create");
    if (snapshot.repairCost != null) {
      await adjustMonthlySales(monthKeyFrom(new Date()), Number(snapshot.repairCost), 1);
    }
  } catch (err) {
...
export async function updateTicket(p) {
  ...
  try {
    ...
    if (repairCost !== oldRepairCost) {
      const delta = (repairCost == null ? 0 : Number(repairCost)) - (oldRepairCost == null ? 0 : Number(oldRepairCost));
      // Credit the month the sale actually happened in (the ticket's
      // creation month), not the month the correction was made.
      await adjustMonthlySales(monthKeyFrom(current.created_at), delta, 0);
    }
  } catch (err) {
    if (newInventoryMovement) await restockInventoryItem(newInventoryMovement.key, p.id).catch(() => {});
