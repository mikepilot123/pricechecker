...
export async function restoreBackup(p) {
  ...
  for (const t of target) {
    ...
    if (existing.length) {
      ...
      await sql`
        INSERT INTO tickets (id, customer_name, phone, device, issues, status, notes, created_at)
        VALUES (${t.id}, ${snap.customerName}, ${snap.phone}, ${snap.device}, ${snap.issues}, ${snap.status}, ${snap.notes}, ${t.created || new Date().toISOString()})
      `;
