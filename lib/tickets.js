...
// Only the new "still untouched after intake" workflow should create these
// automatic repair-check reminders. Older backlog repairs already have their
// own history, so leave them alone.
const REPAIR_CHECK_REMINDER_START = "2026-08-20T01:00:00.000Z";
...
async function syncRepairCheckReminders() {
  await sql`
    ...
    SELECT
      ...
      'This repair has been open for over 1 day. Check the ticket and update the status or due date.',
      now(),
      FALSE,
    ...
    WHERE t.deleted_at IS NULL
      ...
      AND t.created_at >= ${REPAIR_CHECK_REMINDER_START}::timestamptz
