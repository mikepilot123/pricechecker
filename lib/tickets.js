...
export async function addTicket(p) {
  ...
  await sql`
    INSERT INTO tickets (id, customer_name, phone, device, issues, status, notes, repair_cost, amount_paid)
    VALUES (${id}, ${snapshot.customerName}, ${snapshot.phone}, ${snapshot.device}, ${snapshot.issues}, ${snapshot.status}, ${snapshot.notes}, ${snapshot.repairCost}, ${snapshot.amountPaid})
  `;
...
export async function updateTicket(p) {
  ...
  await sql`
    ...
    SET customer_name = ${customerName}, phone = ${phone}, device = ${device},
        ...
        repair_cost = ${repairCost}, amount_paid = ${amountPaid},
        updated_at = now(), current_version = ${nextVersion}
    WHERE id = ${p.id}
  `;
  if (changed) {
    await insertVersion(p.id, nextVersion, lines.join("; "), "update", {
      customerName, phone, device, issues, status, notes, repairCost, amountPaid,
    });
  }
...
export async function restoreBackup(p) {
  ...
  for (const t of target) {
    ...
    if (existing.length) {
      ...
      await sql`
        UPDATE tickets
        SET customer_name = ${snap.customerName}, phone = ${snap.phone}, device = ${snap.device},
            issues = ${snap.issues}, status = ${snap.status}, notes = ${snap.notes},
            repair_cost = ${snap.repairCost}, amount_paid = ${snap.amountPaid},
      ...
      await sql`
        INSERT INTO tickets (id, customer_name, phone, device, issues, status, notes, repair_cost, amount_paid, created_at)
        VALUES (${t.id}, ${snap.customerName}, ${snap.phone}, ${snap.device}, ${snap.issues}, ${snap.status}, ${snap.notes}, ${snap.repairCost}, ${snap.amountPaid}, ${t.created || new Date().toISOString()})
      `;
