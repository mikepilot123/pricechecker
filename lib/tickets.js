...
async function syncRepairCheckReminders() {
  await sql`
    ...
    WHERE t.deleted_at IS NULL
      AND t.status IN ('Received', 'Diagnosing', 'Waiting for Parts', 'Part to be Ordered', 'Part Ordered', 'In Progress')
  ...
  await sql`
    ...
    WHERE r.id = ${REPAIR_CHECK_REMINDER_PREFIX} || t.id
      ...
      AND r.done = FALSE
      AND (t.deleted_at IS NOT NULL OR t.status NOT IN ('Received', 'Diagnosing', 'Waiting for Parts', 'Part to be Ordered', 'Part Ordered', 'In Progress'))
  `;
}
