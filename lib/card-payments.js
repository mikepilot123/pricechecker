...
export async function accountSummary() {
  ...
  const [reclaim] = await sql`
    SELECT COALESCE(SUM(amount - COALESCE(reclaimed_amount, CASE WHEN reclaimed_at IS NOT NULL THEN amount ELSE 0 END)), 0) AS total, COUNT(*) AS count
    FROM expenses
    WHERE deleted_at IS NULL AND cash_reclaim = TRUE
      AND amount > COALESCE(reclaimed_amount, CASE WHEN reclaimed_at IS NOT NULL THEN amount ELSE 0 END)
  `;
