...
export async function stockPartsOrderToInventory({
  partsOrderId,
  itemKey,
  section,
  item,
  ...
  } = await import("./parts-orders.js");
  const id = String(partsOrderId || "").trim();
  const claim = await claimPartsOrderForInventory({ partsOrderId: id });
  if (claim.alreadyStocked) {
    return { alreadyStocked: true, partsOrder: claim.partsOrder, ...(await listInventory()) };
  }
  try {
    ...
    let movement = null;
    if (key) {
      movement = await adjustInventoryItem(key, quantity, { reason: `Parts order ${id}` });
      ...
      inventory = await addInventoryItem({ section: sectionName, item: itemName, quality: qualityName, quantity });
      const created = inventory.items.find((candidate) =>
        candidate.section === sectionName.toUpperCase() &&
        candidate.item.toLowerCase() === itemName.toLowerCase() &&
        String(candidate.quality || "").toLowerCase() === qualityName.toLowerCase());
    ...
    const partsOrder = await completePartsOrderInventory({
      partsOrderId: id, itemKey: key, itemLabel, quantity,
    });
    return { alreadyStocked: false, movement, partsOrder, ...inventory };
  } catch (err) {
    await releasePartsOrderInventoryClaim({ partsOrderId: id });
    throw err;
  }
}
