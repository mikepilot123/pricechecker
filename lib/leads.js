...
async function getLead(id) {
  const rows = await sql`
    SELECT leads.*, (SELECT COUNT(*) FROM lead_notes WHERE lead_notes.lead_id = leads.id) AS notes_count
...
export async function listLeadNotes(leadId) {
  ...
  const rows = await sql`SELECT * FROM lead_notes WHERE lead_id = ${id} ORDER BY created_at DESC`;
...
export async function addLeadNote(p) {
  ...
  await sql`INSERT INTO lead_notes (lead_id, note) VALUES (${leadId}, ${note})`;
