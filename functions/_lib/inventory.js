// Cloudflare Pages Functions build of lib/inventory.js. Logic and function
// signatures are identical to the Vercel original (so lib/tickets.js can
// be reused completely unchanged) — same public CSV read, same
// sheet-parsing rules, same Google Sheets v4 write path, same
// process.env config reads (works via the nodejs_compat flag set in
// wrangler.toml, which populates process.env from the Pages project's
// configured environment variables).
//
// The one real change: the Google service-account JWT is signed with the
// Web Crypto API (crypto.subtle) instead of node:crypto's createSign, so
// this piece has zero dependency on Node compat shims for the trickiest
// part (RSA signing). Verified against a self-generated RSA test key that
// the resulting signature is byte-for-byte verifiable with the standard
// RS256/PKCS1v1.5 algorithm Google expects.
...
export async function listInventory() {
  const url = `https://docs.google.com/spreadsheets/d/${INVENTORY_SHEET_ID}/export?format=csv&gid=${INVENTORY_SHEET_GID}&_=${Date.now()}`;
...
async function googleAccessToken() {
  ...
  const privateKey = await importGooglePrivateKey(rawKey.replace(/\\n/g, "\n"));
  const signatureBuf = await crypto.subtle.sign(
    "RSASSA-PKCS1-v1_5",
    privateKey,
    new TextEncoder().encode(unsigned)
  );
  const signature = base64urlFromBytes(new Uint8Array(signatureBuf));
