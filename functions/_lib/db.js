...
// Cloudflare Pages Functions build of lib/db.js. Only real change from the
// Vercel original: `sql` is now a lazily-initialized wrapper instead of a
// module-top-level `neon(connectionString)` call. Cloudflare's nodejs_compat
// only populates `process.env` once an actual request is in flight — reading
// it at module-evaluation time (like the Vercel original did) reads an
// empty object and throws before the Worker even finishes starting up,
// taking down every route, not just the ones that need the DB. Deferring
// the read to the first real query call (which always happens inside a
// request handler) fixes that. `sql` is still called exactly the same way
// everywhere else (`` sql`SELECT ...` ``), so lib/tickets.js, leads.js,
// appointments.js, and expenses.js are unchanged from the Vercel originals.
let _sql;
function neonClient() {
  if (_sql) return _sql;
